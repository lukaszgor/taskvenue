...
const ManagerVenueAttachments = () => {
  ...
  const fetchDocuments = async () => {
    try {
      const { data, error } = await supabase.storage.from('venue').list(idConfig + '/' + id + '/');
      if (error) {
  ...
  const displayFile = async (file) => {
    const fileUrl = `${supabaseUrl}/storage/v1/object/public/venue/${idConfig}/${id}/${file.name}`;
    window.open(fileUrl);
  };
...
export default ManagerVenueAttachments;
