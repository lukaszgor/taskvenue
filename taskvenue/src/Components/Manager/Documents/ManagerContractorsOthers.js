...
const ManagerContractorsOthers = () => {
  ...
  const fetchDocuments = async () => {
    try {
      const { data, error } = await supabase.storage.from('contractorsothers').list(idConfig + '/' + id + '/');
      if (error) {
  ...
  const displayFile = async (file) => {
    const fileUrl = `${supabaseUrl}/storage/v1/object/public/contractorsothers/${idConfig}/${id}/${file.name}`;
    window.open(fileUrl);
  };
...
export default ManagerContractorsOthers;
