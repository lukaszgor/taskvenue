...
const ManagerEmployeeDocuments = () => {
  ...
  const fetchDocuments = async () => {
    try {
      const { data, error } = await supabase.storage.from('employeeDocuments').list(idConfig + '/' + id + '/');
      if (error) {
  ...
  const displayFile = async (file) => {
    const fileUrl = `${supabaseUrl}/storage/v1/object/public/employeeDocuments/${idConfig}/${id}/${file.name}`;
    window.open(fileUrl);
  };
...
export default ManagerEmployeeDocuments;
