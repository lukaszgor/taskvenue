...
const WorkerConstantWorkingSheetRaport = () => {
  ...
  useEffect(() => {
    if (idConfig) {
      fetchWorkTime(idConfig,userID,daysAgo,currentDate)

    }
  }, [idConfig, userID,daysAgo, currentDate]);


  const fetchWorkTime = async (idConfiguration,userID,fromDate, toDate) => {
    const { data, error } = await supabase
      ...
      .eq('assigned_user', userID).gte('created_at', fromDate).lte('created_at', toDate);
...
export default WorkerConstantWorkingSheetRaport;
